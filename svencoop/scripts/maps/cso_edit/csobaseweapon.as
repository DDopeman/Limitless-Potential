...
class CBaseCSOWeapon : ScriptBasePlayerWeaponEntity
{
	...
	void AdvancedMuzzleflash( int iAttachment, string sprite, float scale, float rotation, float fps, int isalpha, int unknown1, int unknown2, float unknown3, int unknown4, float unknown5, int unknown6 )
	{
		...
		CSprite@ pMuzzleflash = g_EntityFuncs.CreateSprite( "sprites/cso_edit/" + sprite, pev.origin, true, fps );
				...
				if( tr.iHitgroup == HITGROUP_SHIELD )
				{
					...
					if( tr.flFraction != 1.0 )
					{
						if( Math.RandomLong(0, 1) == 1 )
							g_SoundSystem.EmitSound( pEntity.edict(), CHAN_VOICE, "cso_edit/ric_metal-1.wav", 1, ATTN_NORM );
						else
							g_SoundSystem.EmitSound( pEntity.edict(), CHAN_VOICE, "cso_edit/ric_metal-2.wav", 1, ATTN_NORM );
