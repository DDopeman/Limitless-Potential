...
const int CSOW_MAX_CLIP2							= 20;
const int CSOW_MAX_AMMO							    = 720;
const int CSOW_TRACERFREQ							= 2;
const float CSOW_DAMAGE								= 22; //22-30
const float CSOW_DAMAGE2							= 30;
const float CSOW_TIME_DELAY1						= 0.1025;
const float CSOW_TIME_DELAY2						= 0.3;
const float CSOW_TIME_DELAY3						= 6.0; //switch RPM
const float CSOW_TIME_DRAW						= 0.75;
const float CSOW_TIME_IDLE							= 20.0;
const float CSOW_TIME_RELOAD					= 3.6;
...
const int SCAR_SLOT                      = 5;
const int SCAR_POSITION                  = 20;
const int SCAR_WEIGHT                    = 25;

const string CSOW_ANIMEXT							= "m16";

const string MODEL_VIEW								= "models/cso_edit/v_scar_limit.mdl";
const string MODEL_PLAYER							= "models/cso_edit/p_scar.mdl";
const string MODEL_WORLD							= "models/cso_edit/w_scar.mdl";
const string MODEL_SHELL							= "models/cso_edit/pshell.mdl";
const string MODEL_AMMO                             = "models/w_9mmarclip.mdl";

const string SCAR_L_SND								= "cso_edit/scar_l.wav";
const string SCAR_H_SND								= "cso_edit/scar_h.wav";
const string SCAR_DRYFIRE_SND                      = "custom_weapons/cs16/dryfire_rifle.wav";
...
class weapon_scar : CBaseCSOWeapon
{
	...
	void Precache()
	{
		...
		g_Game.PrecacheGeneric( "sprites/cso_edit/weapon_scar.txt" );
        ...
        g_SoundSystem.PrecacheSound( SCAR_L_SND );
		g_SoundSystem.PrecacheSound( SCAR_H_SND );
        g_SoundSystem.PrecacheSound( SCAR_DRYFIRE_SND );
	}
	...
	bool GetItemInfo( ItemInfo& out info ) // Weapon information goes here
	{
		...
		info.iMaxClip 	= (m_iMode == MODE_556) ? CSOW_MAX_CLIP : CSOW_MAX_CLIP2; //Weapon's primary magazine
		info.iAmmo1Drop	= (m_iMode == MODE_556) ? CSOW_MAX_CLIP : CSOW_MAX_CLIP2; //How much ammo to drop
		info.iAmmo2Drop	= -1; //How much secondary ammo to drop
		info.iSlot   	= SCAR_SLOT; //Weapon's slot
		info.iPosition 	= SCAR_POSITION; //Weapon's position on the weapon bucket
		info.iFlags  	= 0; //Weapon's flags
		info.iWeight 	= SCAR_WEIGHT; //Weapon's weight
		return true;
	}
	...
	bool AddToPlayer( CBasePlayer@ pPlayer )
	{
		...
		NetworkMessage m( MSG_ONE, NetworkMessages::WeapPickup, pPlayer.edict() );
			m.WriteLong( g_ItemRegistry.GetIdForName("weapon_scar") );
		m.End();
	...
	bool PlayEmptySound()
	{
		if( self.m_bPlayEmptySound )
		{
			...
			g_SoundSystem.EmitSound( m_pPlayer.edict(), CHAN_WEAPON, SCAR_DRYFIRE_SND, VOL_NORM, ATTN_NORM );
		}
	...
	bool Deploy()
	{
		bool bResult;
		{
			...
			bResult = self.DefaultDeploy( self.GetV_Model(MODEL_VIEW), self.GetP_Model(MODEL_PLAYER), (m_iMode == MODE_556) ? ANIM_DRAW : ANIM_SCAR_H_DRAW, CSOW_ANIMEXT, 0, (m_bSwitchHands ? g_iCSOWHands : 0) );
...
*/
	...
	void PrimaryAttack()
	{
		...
		if( m_iMode == MODE_556 )
		{
			if( !USE_CSLIKE_RECOIL )
			{
				...
				g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, SCAR_L_SND, VOL_NORM, ATTN_NORM, 0, 94 + Math.RandomLong(0, 15) );
			...
			else
			{
				if( !m_pPlayer.pev.FlagBitSet(FL_ONGROUND) )
					SCARFire( 0.035 + (0.4) * m_flAccuracy, CSOW_TIME_DELAY1 );
				else if( m_pPlayer.pev.velocity.Length2D() > 140 )
					SCARFire( 0.035 + (0.07) * m_flAccuracy, CSOW_TIME_DELAY1 );
				else if( m_pPlayer.pev.fov == 0 )
					SCARFire( (0.02) * m_flAccuracy, CSOW_TIME_DELAY1 );
				else
					SCARFire( (0.02) * m_flAccuracy, 0.135 );
			}
		}
		else if( m_iMode == MODE_762 )
		{
			if( !USE_CSLIKE_RECOIL )
			{
				HandleAmmoReduction( 1 );
				...
				m_pPlayer.SetAnimation( PLAYER_ATTACK1 );
				m_pPlayer.pev.effects = int(m_pPlayer.pev.effects) | EF_MUZZLEFLASH; //Needed??
				self.SendWeaponAnim( Math.RandomLong(ANIM_SCAR_H_SHOOT1, ANIM_SCAR_H_SHOOT3), 0, (m_bSwitchHands ? g_iCSOWHands : 0) );
				g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, SCAR_H_SND, VOL_NORM, ATTN_NORM, 0, 94 + Math.RandomLong(0, 15) );
				...
				int iPenetration = USE_PENETRATION ? 2 : 1;
				FireBullets3( vecSrc, g_Engine.v_forward, GetWeaponSpread(), iPenetration, BULLET_PLAYER_762MM, CSOW_TRACERFREQ, CSOW_DAMAGE2, 1.0, 0, CSOW_OFFSETS_MUZZLE );
	...
	void SCARFire( float flSpread, float flCycleTime )
	{
		m_bDelayFire = true;
		m_iShotsFired++;
		...
		{
			FireBullets3( vecSrc, g_Engine.v_forward, flSpread, iPenetration, BULLET_PLAYER_762MM, CSOW_TRACERFREQ, CSOW_DAMAGE2, 0.96 );
	...
	void SecondaryAttack()
	{
		...
		{
			m_iMode = MODE_762;
			g_EngineFuncs.ClientPrintf( m_pPlayer, print_center, " Switched to 7.62MM Mode \n" );
			self.SendWeaponAnim( ANIM_CHANGE_SCAR_H, 0, (m_bSwitchHands ? g_iCSOWHands : 0) );
		}
		else
		{
			m_iMode = MODE_556;
			g_EngineFuncs.ClientPrintf( m_pPlayer, print_center, " Switched to 5.56MM Mode \n" );
			self.SendWeaponAnim( ANIM_CHANGE_SCAR_L, 0, (m_bSwitchHands ? g_iCSOWHands : 0) );
		}

		self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + CSOW_TIME_DELAY3;
		self.m_flTimeWeaponIdle = g_Engine.time + CSOW_TIME_DELAY3 + 0.5;
	}
	...
	void Reload()
	{
		...
		{
			if( m_pPlayer.m_rgAmmo(self.m_iPrimaryAmmoType) <= 0 or self.m_iClip >= CSOW_MAX_CLIP2 or (m_pPlayer.pev.button & IN_ATTACK) != 0 )
				return;
		}
		...
		self.DefaultReload( (m_iMode == MODE_556) ? CSOW_MAX_CLIP : CSOW_MAX_CLIP2, (m_iMode == MODE_556) ? ANIM_RELOAD : ANIM_SCAR_H_RELOAD, CSOW_TIME_RELOAD, (m_bSwitchHands ? g_iCSOWHands : 0) );
	...
	void WeaponIdle()
	{
		...
		self.SendWeaponAnim( (m_iMode == MODE_556) ? ANIM_IDLE : ANIM_SCAR_H_IDLE, 0, (m_bSwitchHands ? g_iCSOWHands : 0) );
	}
...
// Ammo class
class SCAR_MAG : ScriptBasePlayerAmmoEntity
{
	void Spawn()
	{ 
...
void Register()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "cso_scar::weapon_scar", GetName() );
    g_CustomEntityFuncs.RegisterCustomEntity( "cso_scar::SCAR_MAG", GetAmmoName() );
	g_ItemRegistry.RegisterWeapon( GetName(), "cso_edit", AMMO_TYPE, "", GetAmmoName() );
}

} //namespace cso_scar END