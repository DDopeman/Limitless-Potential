...
void MapActivate()
{
	...
	{
		do( g_EntityFuncs.Remove( g_EntityFuncs.FindEntityByTargetname( null, "percent_*" ) ) );
        while( g_EntityFuncs.FindEntityByTargetname( null, "percent_*" ) !is null );

		do( g_EntityFuncs.Remove( g_EntityFuncs.FindEntityByTargetname( null, "skull_*" ) ) );
        while( g_EntityFuncs.FindEntityByTargetname( null, "skull_*" ) !is null );
	}
}
// Horrendous code incoming
array<Vector>
    old_ply_posis( g_Engine.maxClients + 1 ),
...
bool isInside(Vector origin)
{
	...
	int count = 0, i = 0, n = play_area_instinct_9.length();
    do
    {
        int next = (i+1)%n;
...
{
    int n = play_area_instinct_9.length(), best_idx = -1;
    ...
    return best_idx < 1 ? 0 : play_area_instinct_9[best_idx].z;
}

void BlockerThink(CBaseEntity@ pTriggerScript)
{
    for( int iPlayer = 1; iPlayer <= g_Engine.maxClients; ++iPlayer )
    {
        ...
        Vector quad_check = pPlayer.pev.origin;
        quad_check.x -= 16.0;
        quad_check.y -= 16.0;
        ...
        if( !isInside(quad_check) )
        {
            ...
            pPlayer.pev.velocity.x = pPlayer.pev.velocity.y = 0.0;
