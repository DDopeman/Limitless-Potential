...
{
    // No idea why, and looks stupid, but this fixes some string's formatting
    string FixValue( dictionaryValue@ pValue )
    {
        return string( string_t( string( pValue ) ) );
    }

    // Get the correct language string from this dictionary for this player
    ...
    // Ejecutes a client side command on the given player's index or to all players if CMKPlayerFuncs_ALL_PLAYERS (0)
    ...
    */
    ...
    // Use this method instead of GetPlayerAuthId, so BOTS have a unique ID based on their entity index
    ...
    // Convenience function for respawning players with their current inventory
